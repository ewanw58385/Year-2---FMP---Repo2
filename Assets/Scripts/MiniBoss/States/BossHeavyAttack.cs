    ...
    public override void UpdateLogic()
    {
        base.UpdateLogic();
            ...
            if (_bfsm.bossAnim.GetCurrentAnimatorStateInfo(0).normalizedTime >= 0.3f) //if anim is at point of hitting player 
            {
                ...
                {
                    Collider2D[] playerToDamage = Physics2D.OverlapCircleAll(_bm.heavyAttackPosition.position, _bm.heavyAttackRange, _bm.playerLayerMask); //creates array of colliders which are within the boundary and are of the correct layermask 
                    ...
                    foreach (Collider2D playerHit in playerToDamage) //for each enemy hit in the array declared above
                    {
                        ...
                        _bfsm.player.GetComponent<PlayerCombatManager>().TakeDamage(_bm.heavyAttackDamage); //apply damage to player passing weak attack damage as parameter
                    }
        ...
        if (_bfsm.bossAnim.GetCurrentAnimatorStateInfo(0).normalizedTime >= 0.9f) //if anim finished
        {
